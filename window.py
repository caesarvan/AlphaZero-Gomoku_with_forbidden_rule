...
class GomokuWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.setWindowTitle('AlphaZero Gomoku with Forbidden rules')
        self.setFixedSize(50 + self.H * 40, 50 + self.W * 40)

        title = QLabel(self)
        title.setText('AlphaZero五子棋')
        title.move((self.H) * 17, 0)

        author = QLabel(self)
        author.setText('Fan Gaoyige')
        author.move((self.H - 1) * 40, self.W * 40)

        id = QLabel(self)
        id.setText('1728405039')
        id.move((self.H - 1) * 40, 20+self.W * 40)
        #print(50+self.H*40 ,50+self.W*40)
        #        self.setStyleSheet('#MainWindow{background-color: green}')
        palette = QPalette()
        # palette.setBrush(QPalette.Window, QBrush(QPixmap('img/board.jpg')))
